...
var Stitcher = (function() {
  function Stitcher(config) {
    ...
    if (_.isString(config.local_path)) {
      ...
      fs.readdirSync(config.local_path).forEach(function(versionDir) {
        me.versions[versionDir] = Stitcher.loadVersion(config.local_path, versionDir);
      });
    }
  };

  Stitcher.moduleFromConfigHash = function(moduleHash, moduleName, versionDir) {
    var tmpModule = {};
  ...
  Stitcher.loadVersion = function(local_path, loadVersionName) {
    ...
    */
    ...
    _.each(manifest.common, function(moduleHash, moduleName) {
      tmpModule = Stitcher.moduleFromConfigHash(moduleHash, moduleName, newVersion.dir);
      if (tmpModule !== null) {
  ...
  Stitcher.prototype.stitch = function(selector, stitchVersionName, stitchCallback) {
  ...
  Stitcher.prototype.getVersion = function(getVersionName, getVersionCallback) {
    ...
    http.get({
      ...
      response.on('end', function() {
        ...
        exec('cd '+me.config.local_path+'/'+'; tar -xzf '+filename+'; rm '+filename,
          ...
          // Load the version
          me.versions[getVersionName] = Stitcher.loadVersion(me.config.local_path, getVersionName);
